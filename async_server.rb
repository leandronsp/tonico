...
accept = Fiber.new do |io|
  loop do 
    ...
    controller = Fiber.new do |request, params|
      conn = pool.checkout

      db = Fiber.new do |conn, sql, params|
        loop do 
          result = conn.exec_params(sql, params).to_a
          conn, sql, params = Fiber.yield(result)
        end
      end
      ...
      in 'GET /clientes/:id/extrato'
        ...
        ten_transactions = db.resume(conn, sql_ten_transactions, [params['id']])
    ...
    @fibers << [controller, [request, params]]
